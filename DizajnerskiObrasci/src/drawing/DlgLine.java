...
public class DlgLine extends JDialog {
	...
	private JTextField txtxInitial;
	private JTextField txtyInitial;
	
	private JTextField txtxLast;
	private JTextField txtyLast;
	
	private JLabel lblxInitial;
	...
	public DlgLine() {
		...
		{
			lblxInitial = new JLabel("X coordinate of initial point");
			lblxInitial.setFont(new Font("Arial", Font.BOLD, 12));
			...
			contentPanel.add(lblxInitial, gbc_lblxCoordinateInitial);
		}
		{
			txtxInitial = new JTextField();
			lblxInitial.setLabelFor(txtxInitial);
			txtxInitial.setFont(new Font("Arial", Font.BOLD, 12));
			...
			contentPanel.add(txtxInitial, gbc_txtxCoordinateInitial);
			txtxInitial.setColumns(10);
		}
		{
			...
			contentPanel.add(lblyCoordinateInitial, gbc_lblyCoordinateInitial);
		}
		{
			txtyInitial = new JTextField();
			lblyCoordinateInitial.setLabelFor(txtyInitial);
			txtyInitial.setFont(new Font("Arial", Font.BOLD, 12));
			...
			contentPanel.add(txtyInitial, gbc_txtyCoordinateInitial);
			txtyInitial.setColumns(10);
		}
		{
			...
			contentPanel.add(lblxCoordinateLast, gbc_lblxCoordinateLast);
		}
		{
			txtxLast = new JTextField();
			lblxCoordinateLast.setLabelFor(txtxLast);
			...
			contentPanel.add(txtxLast, gbc_txtxCoordinateLast);
			txtxLast.setColumns(10);
		}
		{
			...
			contentPanel.add(lblyCoordinateLast, gbc_lblyCoordinateLast);
		}
		{
			txtyLast = new JTextField();
			lblyCoordinateLast.setLabelFor(txtyLast);
			...
			contentPanel.add(txtyLast, gbc_txtyCoordinateLast);
			txtyLast.setColumns(10);
		}
			...
			getContentPane().add(buttonsPanel, BorderLayout.SOUTH);
			{
				...
				btnConfirm.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent click) {
						if (txtxInitial.getText().isEmpty() || txtyInitial.getText().isEmpty()  || txtxLast.getText().isEmpty() || txtyLast.getText().isEmpty()) JOptionPane.showMessageDialog(getParent(), "Values cannot be empty!", "Error", JOptionPane.ERROR_MESSAGE);
						else {
							try {	
								xCoordinateInitial = Integer.parseInt(txtxInitial.getText());
								yCoordinateInitial = Integer.parseInt(txtyInitial.getText());
								xCoordinateLast = Integer.parseInt(txtxLast.getText());
								yCoordinateLast = Integer.parseInt(txtyLast.getText());
								if (xCoordinateInitial <= 0 || yCoordinateInitial <= 0 || xCoordinateLast <= 0 || yCoordinateLast <= 0) 
									JOptionPane.showMessageDialog(null, "X and Y coordinates of initial and last point of line must be positive numbers!");
								else {
									confirmed = true;
									setVisible(false);
									dispose();
	...
	public void write(Line line) {
		txtxInitial.setText(String.valueOf(line.getInitial().getXcoordinate()));
		txtyInitial.setText(String.valueOf(line.getInitial().getYcoordinate()));
		txtxLast.setText(String.valueOf(line.getLast().getXcoordinate()));
		txtyLast.setText(String.valueOf(line.getLast().getYcoordinate()));
